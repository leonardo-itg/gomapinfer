...
type OSMOptions struct {
	...
	IncludeRailway  bool
	BuildingEdges   []map[int]bool
	BuildingCentres []map[int]bool
	DoRoads         bool
}
...
func LoadOSMMultiple2(path string, regions []Rectangle, options OSMOptions) ([]*Graph, error) {
	...
	fmt.Printf("Starting grid index\n")
	...
	fmt.Printf("Starting regionIndex\n")
	...
	if options.DoRoads == true {
		fmt.Printf("Parsing roads...")
		...
		fmt.Printf("Parsing Buildings")
		count = 0
		err = DecodeOSM(path, options, func(v interface{}) {
			switch v := v.(type) {
			case *osmpbf.Way:
				_, ok := v.Tags["building"]
				if !ok {
					return
				} else if len(v.NodeIDs) < 2 {
					return
				...
				if len(options.BuildingCentres) > 0 {
					for _, redge := range wayEdges {
						options.BuildingCentres[redge.RegionID][redge.Edge.ID] = true
					}
				}

				if len(options.BuildingEdges) > 0 {
					for _, redge := range wayEdges {
						options.BuildingEdges[redge.RegionID][redge.Edge.ID] = true
					}
				}
